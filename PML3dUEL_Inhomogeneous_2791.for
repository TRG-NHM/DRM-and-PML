...
!      PML_b = PML_L / 1.d0   !characteristic length (average element size in the PML domain) 
      PML_b = PML_L * 5.d0   !characteristic length (10 average element size in the PML domain) 
      
      ! NOTE: cp_ref is the maximum Vp in the PML domain.
      ! For inhomogeneous soil box, this is critical and has to be assigned manually.
      cp_ref = 2790.5194299475884d0
      ! NOTE 2: Read cp_ref from file
      ! open(1, file='maximumVpInPMLDomain.txt')
      ! read(1, *) cp_ref
      ! close(1)
