...
def createPreliminaryModel(lengths, DRM_depth, PML_depth, partName, materialName, 
    ...
    if isStaticStepIncluded:
        ...
        model.Gravity(name='Gravity', createStepName=staticStep.name, 
            comp3=9.81, amplitude=amp.name, distributionType=UNIFORM, field='')
...
if __name__ == '__main__':
    ...
    createPreliminaryModel(lengths, *[data[key] for key in dataKeysForPreliminaryModel], noDamping=True, stepApplication='moderate dissipation', isStaticStepIncluded=False)
    setViews()